...
There are demos for **Symfony 6**, **7**, **8** and **8 (PHP 8.5)** (e.g. **demo/symfony6**, **demo/symfony7**, **demo/symfony8**, **demo/symfony8**). Each has its own Dockerfile, docker-compose.yml and Makefile. From the bundle root you run e.g. `make -C demo/symfony8 up` (see the demo's README for the URL and port).
...
- **Phone Input Bundle** (`Nowo\PhoneInputBundle\NowoPhoneInputBundle`) — the bundle under test; enabled in the demos.
...
Each demo's **docker-compose.yml** sets `APP_ENV=dev` and `APP_DEBUG=1`, mounts the app, the bundle (`../..:/var/phone-input-bundle`), `docker/frankenphp/Caddyfile.dev`, and `docker/php-dev.ini`. The entrypoint copies Caddyfile.dev when `APP_ENV=dev`.

### 5. Start the demo (development)

From the bundle root: `make -C demo/symfony8 up` (or symfony7, symfony6, symfony8). Or from the demo directory: `make up`.
